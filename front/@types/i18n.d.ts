// Internal Helpers
declare type $NormalizeIntoArray<T extends unknown | readonly unknown[]> =
  T extends readonly unknown[] ? T : [T];


declare type $MergeBy<T, K> = Omit<T, keyof K> & K;
 ...
 * ```
 */
declare interface CustomTypeOptions {}

declare type TypeOptions = $MergeBy<
  {
    /**
...
declare type $PreservedValue<Value, Fallback> = [Value] extends [never] ? Fallback : Value;
...
declare type DefaultNamespace = TypeOptions['defaultNS'];
declare type FlatNamespace = $PreservedValue<keyof TypeOptions['resources'], string>;
...
declare type PluginOptions<T> = $MergeBy<
  {
    /**
...
declare interface InitOptions<T = object> extends PluginOptions<T> {
    /**
  ...
  declare interface TOptionsBase {
    /**
...
declare type Namespace<T = FlatNamespace> = T | readonly T[];

declare type ResourceKey =
  | string
  | {
      [key: string]: any;
    };

declare interface ResourceLanguage {
...
declare type _ReturnObjects = TypeOptions['returnObjects'];
declare type _ReturnEmptyString = TypeOptions['returnEmptyString'];
declare type _ReturnNull = TypeOptions['returnNull'];
declare type _KeySeparator = TypeOptions['keySeparator'];
declare type _NsSeparator = TypeOptions['nsSeparator'];
declare type _PluralSeparator = TypeOptions['pluralSeparator'];
declare type _ContextSeparator = TypeOptions['contextSeparator'];
declare type _FallbackNamespace = TypeOptions['fallbackNS'];
declare type _Resources = TypeOptions['resources'];
declare type _JSONFormat = TypeOptions['jsonFormat'];
declare type _InterpolationPrefix = TypeOptions['interpolationPrefix'];
declare type _InterpolationSuffix = TypeOptions['interpolationSuffix'];

declare type ParseKeysByKeyPrefix<Keys, KPrefix> = KPrefix extends string
  ? Keys extends `${KPrefix}${_KeySeparator}${infer Key}`
    ? Key
    : never
  : Keys; 
declare type $IsResourcesDefined = [keyof _Resources] extends [never] ? false : true;

declare type $FirstNamespace<Ns extends Namespace> = Ns extends readonly any[] ? Ns[0] : Ns;

declare type AppendKeyPrefix<Key, KPrefix> = KPrefix extends string
? `${KPrefix}${_KeySeparator}${Key & string}`
: Key;

declare interface TFunction<Ns extends Namespace = DefaultNamespace, KPrefix = undefined> {
...
declare type PluralSuffix = _JSONFormat extends 'v4'
? 'zero' | 'one' | 'two' | 'few' | 'many' | 'other'
: number | 'plural';

declare type WithOrWithoutPlural<Key> = _JSONFormat extends 'v4' | 'v3'
...
declare type $OmitArrayKeys<Arr> = Arr extends readonly any[] ? Omit<Arr, keyof any[]> : Arr;


declare type JoinKeys<K1, K2> = `${K1 & string}${_KeySeparator}${K2 & string}`;

declare type KeysBuilderWithReturnObjects<Res, Key = keyof Res> = Key extends keyof Res
...
declare type KeysBuilderWithoutReturnObjects<Res, Key = keyof $OmitArrayKeys<Res>> = Key extends keyof Res
  ? Res[Key] extends $Dictionary | readonly unknown[]
    ? JoinKeys<Key, KeysBuilderWithoutReturnObjects<Res[Key]>>
    : Key
  : never;

declare type KeysBuilder<Res, WithReturnObjects> = $IsResourcesDefined extends true
? WithReturnObjects extends true
  ? keyof Res | KeysBuilderWithReturnObjects<Res>
  : KeysBuilderWithoutReturnObjects<Res>
: string;

declare type $ValueIfResourcesDefined<Value, Fallback> = $IsResourcesDefined extends true
? Value
: Fallback;

declare type Resources = $ValueIfResourcesDefined<_Resources, $Dictionary<string>>;


declare type KeysWithReturnObjects = {
  [Ns in FlatNamespace]: WithOrWithoutPlural<KeysBuilder<Resources[Ns], true>>;
};
declare type KeysWithoutReturnObjects = {
  [Ns in FlatNamespace]: WithOrWithoutPlural<KeysBuilder<Resources[Ns], false>>;
};


declare type ResourceKeys<WithReturnObjects = _ReturnObjects> = WithReturnObjects extends true
  ? KeysWithReturnObjects
  : KeysWithoutReturnObjects;

declare type KeyPrefix<Ns extends Namespace> = ResourceKeys<true>[$FirstNamespace<Ns>] | undefined;

declare type $Dictionary<T = unknown> = { [key: string]: T };

declare type KeysByTOptions<TOpt extends TOptions> = TOpt['returnObjects'] extends true
  ? ResourceKeys<true>
  : ResourceKeys;

declare type NsByTOptions<Ns extends Namespace, TOpt extends TOptions> = TOpt['ns'] extends Namespace
  ? TOpt['ns']
  : Ns;

declare type FilterKeysByContext<Keys, Context> = Context extends string
? Keys extends `${infer Prefix}${_ContextSeparator}${Context}${infer Suffix}`
  ? `${Prefix}${Suffix}`
  : never
: Keys;

declare type AppendNamespace<Ns, Keys> = `${Ns & string}${_NsSeparator}${Keys & string}`;


declare type ParseKeysByNamespaces<Ns extends Namespace, Keys> = Ns extends readonly (infer UnionNsps)[]
? UnionNsps extends keyof Keys
  ? AppendNamespace<UnionNsps, Keys[UnionNsps]>
  : never
: never;

declare type ParseKeysByFallbackNs<Keys extends $Dictionary> = _FallbackNamespace extends false
? never
: _FallbackNamespace extends (infer UnionFallbackNs extends string)[]
? Keys[UnionFallbackNs]
: Keys[_FallbackNamespace & string];

declare type ParseKeys<
...
declare interface WithT<Ns extends Namespace = DefaultNamespace> {
...
declare type TFunctionDetailedResult<T = string, TOpt extends TOptions = {}> = {
  /**
   * The plain used key
   */
  usedKey: string;
...
declare type $UnionToIntersection<T> = (T extends unknown ? (k: T) => void : never) extends (
  k: infer I,
) => void
  ? I
  : never;

declare type $StringKeyPathToRecordUnion<
...
declare type $StringKeyPathToRecord<TPath extends string, TValue> = $UnionToIntersection<
$StringKeyPathToRecordUnion<TPath, TValue>
>;

declare type ParseInterpolationValues<Ret> =
...
declare type InterpolationMap<Ret> = $PreservedValue<
...
declare interface ReactOptions {
    /**
...
declare interface FallbackLngObjList {
    [language: string]: readonly string[];
  }
  
declare type FallbackLng =
    | string
    | readonly string[]
    | FallbackLngObjList
...
declare type TFunctionReturnOptionalDetails<Ret, TOpt extends TOptions> = TOpt['returnDetails'] extends true
  ? TFunctionDetailedResult<Ret, TOpt>
  : Ret;

declare type KeyWithContext<Key, TOpt extends TOptions> = TOpt['context'] extends string
? `${Key & string}${_ContextSeparator}${TOpt['context']}`
: Key;

declare type ParseTReturnPlural<
  Res,
  Key,
  KeyWithPlural = `${Key & string}${_PluralSeparator}${PluralSuffix}`,
> = Res[(KeyWithPlural | Key) & keyof Res];

declare type ParseTReturnPluralOrdinal<
  Res,
  Key,
...
declare type ParseTReturnWithFallback<Key, Val> = Val extends ''
...
declare type ParseTReturn<Key, Res, TOpt extends TOptions = {}> = ParseTReturnWithFallback<
Key,
...
declare type $SpecialObject = object | Array<string | object>;

declare type TReturnOptionalNull = _ReturnNull extends true ? null : never;
declare type TReturnOptionalObjects<TOpt extends TOptions> = _ReturnObjects extends true
  ? $SpecialObject | string
  : TOpt['returnObjects'] extends true
  ? $SpecialObject
  : string;

declare type DefaultTReturn<TOpt extends TOptions> = TReturnOptionalObjects<TOpt> | TReturnOptionalNull;

declare type TFunctionReturn<
...
declare type TOptions<TInterpolationMap extends object = $Dictionary> = TOptionsBase &
  TInterpolationMap;
...
declare type CallbackError = Error | string | null | undefined;
declare type ReadCallback = (
  err: CallbackError,
  data: ResourceKey | boolean | null | undefined,
) => void;
declare type MultiReadCallback = (err: CallbackError, data: Resource | null | undefined) => void;
 ...
 */
declare interface BackendModule<Options = object> extends Module {
  type: 'backend';
 ...
 */
declare interface LanguageDetectorModule extends Module {
 ...
 * For singleton set property `type` to `'languageDetector'` For a prototype constructor set static property.
 */
declare interface LanguageDetectorAsyncModule extends Module {
  type: 'languageDetector';
 ...
 */
declare interface PostProcessorModule extends Module {
  /** Unique name */
  name: string;
  type: 'postProcessor';
 ...
 */
declare interface LoggerModule extends Module {
  type: 'logger';
  log(...args: any[]): void;
  warn(...args: any[]): void;
  error(...args: any[]): void;
...
declare interface I18nFormatModule extends Module {
  type: 'i18nFormat';
}

declare interface FormatterModule extends Module, Formatter {
  type: 'formatter';
}

declare interface ThirdPartyModule extends Module {
  type: '3rdParty';
...
declare interface NewableModule<T extends Module> extends Newable<T> {
  type: T['type'];
}

declare type Callback = (error: any, t: TFunction) => void;

/**
 * Uses similar args as the t function and returns true if a key exists.
 */
declare interface ExistsFunction<
...
declare interface CloneOptions extends InitOptions {
  /**
...
declare interface CustomInstanceExtensions {}
...
declare interface i18n extends CustomInstanceExtensions {
  // Expose parameterized t in the i18next interface hierarchy
  t: TFunction<
    [
